		...
		<div className={`${CONTENT_BOX} ${props.className ? props.className  : ''} ${isVisible ? '' : 'invisible'} ${props.outline ? 'outlined' : ''}`} id={id} ref={domRef}>
			<div id={CONTENT_BOX_IMAGE_AND_MAIN+id} className={`${CONTENT_BOX_IMAGE_AND_MAIN}`}>
				...
				<div className={CONTENT_BOX_MAIN}>
					...
					{props.children}
					{props.more ? <>
						<MoreButton expanded={showMore} onClick={setShowMore}/>
					</> : <></>}
				</div>
			</div>
...
function MoreButton(props: {expanded: boolean, onClick: (showMore: boolean)=>void}) {
	return (
		<button className={CONTENT_BOX_MORE_BUTTON} onClick={() => props.onClick(!props.expanded)}>{props.expanded ? "\\/" : "/\\"}</button>
	)
}
